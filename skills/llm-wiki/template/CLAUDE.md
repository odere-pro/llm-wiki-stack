...
When creating a new top-level topic folder, run `/llm-wiki-stack:obsidian-graph-colors` (or the ingest pipeline handles it automatically in step 1.7). The `llm-wiki-stack-curator-agent` also checks for missing color groups and applies them.
